# みっふぃーAIに聞く — Kindleハイライト＋自分の記事の検索

iPhone から「はなしかたでない悩んでいる」のように打つと、これまで読んだ本の
ハイライトと**自分が過去に書いた記事**から答えを探して解決策を出す。
Mac が起動していなくても使える。

上の「両方 / 本だけ / 自分の記事だけ」で探す範囲を切り替える。
本に聞きたいときに自分の記事が混ざらないように、既定は本と記事の両方だが、
書くネタを探すときは「自分の記事だけ」にすると速い。

- 蔵書: Amazon の notebook から 668 冊 / 13,925 ハイライト
- 自分の記事: `personal_ai/corpus/articles/` から 1,309 本（note・ブログ・Substack）
  を 400 字前後に割ったもの
...
iPhone のホーム画面「みっふぃーAI」
  ...
  │     本と記事あわせて 2 万件超と int8 の内積 → 上位 24 件
  │     （1 冊・1 記事あたり 3 件まで）
  │
  └ ③ 答えを書く ────────── Gemini API
...
`personal_ai/collect.py` が note・ブログ・Substack から集めた Markdown が元。
[article_passages.py](article_passages.py) が段落を積んで 400 字前後に割り、
本のハイライトと同じ形にして `build_index.py` に渡す。

記事は 2000 字あるので、まるごと 1 ベクトルにすると話題が混ざって何にも似なくなる。
逆に細かく割りすぎると文脈が消えるので、埋め込みに掛ける文には題名と見出しを前に付けている。

出典テーブル（`meta.json` の `books`）は本と記事の両方を持ち、記事には `k:"a"`、
`u`（URL）、`d`（日付）が付く。PWA はこれを見て「Kindleで開く」と「記事を開く」を出し分ける。

**メール・予定・リマインダーは載せていない。** 合い言葉で暗号化してあるとはいえ、
URL 自体は公開されている。外出先で要るのは記事とハイライトだけなので、
個人的な記録は Mac の中（Claude Code / Discord 経由）に留める。

## 手で作り直す

```bash
.venv/bin/python fetch_kindle_web.py    # Amazon → highlights.jsonl
.venv/bin/python build_index.py         # 埋め込み（本＋記事で約 2 分）
.venv/bin/python build_index.py --no-articles   # 本だけで作りたいとき
